...
class RPC {
	...
	handleError({ id, action, error, socket }) {
		const payload = {
			id,
			action,
			type: 'error',
				...
				if (action && type) {
					if (!existingFunctions) {
						...
						existingFunctions.map((func) => {
							// NOTE - would love to unify sarus and ws as just socket for rpc functions, much simpler
							func({ id, action, type, data, sarus, ws });
						});
					}
				}
			}
