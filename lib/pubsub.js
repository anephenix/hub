...
const clientMustBeSubscriberError = 'You must subscribe to the channel to publish messages to it';
const tooManyWildcardChannelConfigurationsMatchedError = 'Internal error - too many wildcard channel configurations matched the channel';
const wildcardTooAmbiguousError = channel => `Wildcard channel name too ambiguous - will collide with "${channel}"`;
const clientsCannotPublishToChannelError = 'Clients cannot publish to the channel';
...
class PubSub {
	...
	getChannelConfiguration(channel) { 
		...
		if (!channelConfiguration) {
			const wildCardChannelConfigurations = Object.keys(this.channelConfigurations).filter(s => s.match(/\*/));
			if (wildCardChannelConfigurations && wildCardChannelConfigurations.length !== 0) {
				if (wildCardChannelConfigurations.length > 1) {
					throw new Error(tooManyWildcardChannelConfigurationsMatchedError);
				}
				channelConfiguration = this.channelConfigurations[wildCardChannelConfigurations[0]];
			}
		}
	...
	async publish({ data, socket }) {
		...
		if (clientId) {
			if (channelConfiguration && channelConfiguration.clientCanPublish !== undefined) {
				if (typeof channelConfiguration.clientCanPublish === 'boolean' && channelConfiguration.clientCanPublish === false) {
	...
	addChannelConfiguration({ channel, authenticate, clientCanPublish }) {
		...
		this.channelConfigurations[channel] = { authenticate, clientCanPublish };
	}
