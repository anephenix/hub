...
const refreshMessages = () => {
    ...
    const visibleMessages = messages.slice(scrollIndex, scrollIndex + MESSAGE_AREA_HEIGHT);
...
const enableScrolling = () => {
    ...
    terminal.on('mouse', (name, data) => {
        if (data.y < terminal.height - 2) { // Ensure scrolling only affects message area
            if (data.mwheelup) {
                // Scroll up
                scrollIndex = Math.max(0, scrollIndex - 1);
            } else if (data.mwheeldown) {
                // Scroll down
                scrollIndex = Math.min(messages.length - MESSAGE_AREA_HEIGHT, scrollIndex + 1);
            }
            refreshMessages();
        }
    });
...
const readInput = () => {
    terminal.inputField({ cursorPosition: terminal.height - 1 }, async (error, input) => {
        if (error) {
