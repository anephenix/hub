...
Given("a returning client opens a connection to the server", async () => {
	await visitPage("/");
	await closePage();
	await visitPage("/");
});
...
When(
	...
	async (message, channel) =>
		await publishMessageToChannel({ message, channel }),
);

Then(
	"the client should receive the message {string} for the channel {string}",
	async (message, channel) =>
		await clientReceivesMessageForChannel({ message, channel }),
);
...
Then(
	...
	async (message, channel) =>
		await clientDoesNotReceiveMessageForChannel({
			message,
			channel,
		}),
);
...
When("the client unsubscribes from the channel {string}", async (channel) => {
...
Given("I wait for {int} seconds", { timeout: 60000 }, async (int) => {
